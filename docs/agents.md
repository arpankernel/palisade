...
to be pasted into an agent's context or fetched via [`llms.txt`](../llms.txt).
...
[rules-reference.md](rules-reference.md). Keep custom rules in the target
repo (e.g. `security/palisade-rules/`) so the coverage travels with the code.

## For agents working on Palisade itself

See [`AGENTS.md`](../AGENTS.md) at the repo root: build/test commands,
